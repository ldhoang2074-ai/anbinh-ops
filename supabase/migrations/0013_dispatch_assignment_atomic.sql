-- ============================================================
-- 0013_dispatch_assignment_atomic.sql
-- Điều phối an toàn: một assignment ACTIVE cho mỗi Order và
-- một RPC atomic để ghi assignment/order/history/audit.
-- ============================================================

-- Một Order chỉ có thể có một assignment đang hiệu lực.
-- Giữ nguyên exclusion constraints theo xe/tài xế ở 0008.
create unique index if not exists assignments_one_active_per_order_uidx
  on public.assignments (organization_id, order_id)
...
begin
  -- Serialize assignments for the same Order before checking its state.
  select o.*
    into v_order
  ...
  -- Lock candidates so a concurrent soft-delete or status update cannot race
  -- the validation below and the final write.
  select v.*
    into v_vehicle
  ...
  -- Preserve the existing command semantics: only these statuses are blocked.
  ...
  -- Catch inconsistent historical data with an already-active assignment.
  perform 1
...
-- This write RPC is callable only by the server's service-role client.
