...
async function runCommand(name: string, input: unknown, idempotencyKey?: string) {
  ...
  if (!json.ok) throw new Error(json.error || 'Lỗi thao tác');
  return json.data;
}
...
export const LeadRepository = {
  async list() {
    ...
    const { data, error } = await sb.from('leads').select('*').is('deleted_at', null)
...
export const OrderRepository = {
  async list() {
    const sb = createClient();
    const { data, error } = await sb.from('orders').select('*').is('deleted_at', null)
      .order('created_at', { ascending: false });
    if (error) throw error;
    return data;
  },
...
export const DispatchRepository = {
  assign: (orderId: string, vehicleId: string, driverId: string, reason?: string) =>
    runCommand('assign_vehicle_driver', { orderId, vehicleId, driverId, reason }),
};
...
export const FinanceRepository = {
  recordPayment: (input: { orderId: string; amount: number; type?: string; method?: string; reason?: string },
                  idempotencyKey: string) =>
    runCommand('record_payment', input, idempotencyKey),
  recordExpense: (input: { orderId: string; amount: number; category: string; note?: string },
                  idempotencyKey: string) =>
    runCommand('record_expense', input, idempotencyKey),
  async list(table: 'payments' | 'expenses', orderId: string) {
    const sb = createClient();
    const { data, error } = await sb.from(table).select('*').eq('order_id', orderId);
    if (error) throw error;
    return data;
  },
};
...
export const TrafficRepository = {
  async list(sinceDays = 30) {
    ...
    const { data, error } = await sb.from('traffic_events').select('*')
      .gte('timestamp', since).order('timestamp', { ascending: false });
    if (error) throw error;
    return data;
  },
};
